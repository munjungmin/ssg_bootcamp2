...
public class EchoGUIServer extends JFrame implements Runnable{  //나는 프레임이자 run 메서드를 가진 객체 
	JPanel p_north;
	JTextField t_port;
	JButton bt;
	JTextArea area;
	...
	public EchoGUIServer() {
		...
		bt.addActionListener(e -> {
			thread = new Thread(EchoGUIServer.this);   //그냥 this라고 쓰면 람다의 익명객체를 의미하므로 
			thread.start();
		});
